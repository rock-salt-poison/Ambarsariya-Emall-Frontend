...
function ProductSearch() {
  ...
  const fetchSectors = async () => {
    try {
      setLoading(true);
      const selected_domain_id = domains.find(
        (d) => d.domain_name === formData?.domain
      )?.domain_id;
  ...
  const fetchProducts = async (domain, sector, product) => {
    try {
      ...
      const selected_sector_id = sectors?.find(
        (s) => s.sector_name === sector
      )?.sector_id;

      const resp = await get_searched_products(
        selected_domain_id,
        selected_sector_id,
        product
      );
                  ...
                  return (
                    <Box className="card" key={index}>
                      ...
                      <Box className="card_body">
                        <Link
                          className="details"
                          to={`../support/shop/shop-detail/${product?.shop_access_token}`}
                        >
                          <Box
                            component="img"
                            alt="product"
                            src={convertDriveLink(product?.product_images?.[0])}
